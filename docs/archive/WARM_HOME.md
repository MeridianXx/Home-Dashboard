...
**Status / Observera (M1, 2026-05-20):**
- [x] `/api/mat/import` — POST `{url}` → server fetchar HTML (UA-header + `AbortSignal.timeout(12000)`) → extraherar `og:image`/`twitter:image`/JSON-LD `Recipe.image` server-side innan Claude → slimmar HTML (strippar script/style/nav/footer + bevarar JSON-LD-block) → Claude Sonnet 4.6 extraherar strukturerat JSON enligt `ImportedRecipe`-typen. Defensive JSON-parsing (Markdown-fence-stripping + `{`/`}`-bracketing) + default-fallbacks per fält. Returnerar JSON för review (sparar inte). `RATE_LIMIT_AI_EXPENSIVE`-gate.
- [x] `/api/mat/recipes` GET (lista, sorterad på `created_time desc`) + POST (skapa) + `/api/mat/recipes/[id]` GET + PATCH + DELETE (mjuk via Notion `archived: true`). Alla 501-gateas på `isMatReady()`.
- [x] `(warm)/v3/mat/bibliotek/page.tsx` — chip-filter, Importera-knapp (ACC), magazine-grid via `repeat(auto-fill, minmax(280px, 1fr))`, villkorad bild med 16:10 aspect-ratio (ingen emoji-fallback), Fraunces 18 px namn, kursiv lede (2-line clamp), meta-rad (min + ACC-svårighet-dots + 2 ACC-tint tag-pills). Tom-state: "0 recept · importera ditt första" + ACC-knapp.
- [x] Importera-flödet: knapp → URL-modal → "Hämtar…"-spinner → review-modal (samma `RecipeForm`-komponent som manuell create) → "Spara recept" → `mutate('/api/mat/recipes')`. `aiSkapad: true` sätts automatiskt vid import.
- [x] Manuell create-modal — "+ Manuellt"-knapp i bibliotek-headern. Samma `RecipeForm` med tom initial-state, `aiSkapad: false`.
- [x] `(warm)/v3/mat/recept/[id]/page.tsx` — DetailHero (`{TAG} · {min} MIN` eyebrow), villkorad hero-bild (200 px), källa-rad italic "från {kallaLabel} →", lede i italic, **PortionStepper 1–8** (ACC ±-knappar, disablad vid 1/8), ingredienslista i Tile (Fraunces 14 ACC mono-mängd + body 14 namn), steg-kort med ACC-tint cirkel + italic Fraunces-siffra, vintips i LINGON-tinted tile, sticky bottom-bar (Lägg på veckan disablad, Börja laga ACC). `RecipeView` remountar via `key={recipe.id}` för säker portion-state-init utan setState-i-effect.
- [x] Verifierat end-to-end mot `landleyskok.se/recept/stekt-lax-med-vitloksris-och-asiatisk-mangosallad`: Claude extraherade 19 ingredienser + 6 steg + bild + vintips + 4 taggar (Fisk/Ris/Sallad/Vardag) + 40 min + svårighet 1. Skalning 4→5 portioner: 3 msk → 3,8 msk (smart-rounding träffar 1-decimal för 1–10-range). Hero-bild renderar, källa-länk klickbar, sticky bottom syns över TabBar.

**Observera (M1):**
- **Återanvände befintlig Receptbok-DB istället för att skapa ny "Recept"-DB.** Användaren hade redan `🥘 Receptbok` + `📆 Veckomeny` under "Mat & Recept"-kolumnen i Villa Björkdalen. Princip: håll Notion-strukturen ren. Utvidgade Receptbok med 11 nya properties (Lede, Ingredienser, Steg, MinTotal, Svårighet, BasPortioner, Vintips, BildURL, Källa, AISkapad, Taggar) via `notion-update-data-source` DDL. Befintliga properties (Typ, Kategori, Länk, Kommentar, Barnfavorit) lämnades orörda — `Länk` återanvänds som receptkällans URL (matchar befintliga rader). `Kommentar` förblir användarens egen post-cooking-notering, `Lede` är receptkällans ingress (olika saker).
- **"Mat & Recept" är inte en separat sida utan en `# Mat & Recept`-rubrik i Villa Björkdalen-kolumnvy.** Inte tillgänglig som `NOTION_MAT_COACH_PAGE`. Lösning: skapade ny undersida `🧑🏻‍🍳 Kökschefen` under Villa Björkdalen som persona-källa. Page-id finns i `.env.local` + redo för M3.
- **Befintliga DB:er ärver inte connections från parent-page.** `Träningscoach`-integrationen hade access till Villa Björkdalen + Växtregister/Säsongsplan etc, men `Receptbok` + `Veckomeny` (skapade tidigare) krävde manuell `Connections → Add` för var och en. Notion-skriptet `scripts/create-mat-notion-dbs.mjs` döpte tidigare KällURL-fältet — uppdaterade det till `Länk` så schemat matchar befintlig DB om någon kör skriptet i framtiden.
- **Receptbok-mapping: `kallaUrl` → property `Länk`.** Min app:s typsystem behöll fältnamnet `kallaUrl` (RecipeInput), men `recipeProps()` mappar till Notion-property `Länk` istället för `KällURL`. Det ger nollkrock med befintliga rader som redan har `Länk` ifyllt.
- **Importören är defensiv med JSON-parsing.** Claude returnerar ibland ```json-fence eller bifogar förklaring efter `}`. Routen normaliserar genom: (1) stripa Markdown-fence, (2) hitta första `{` och sista `}`, (3) JSON.parse. Defaults fyller i saknade fält (`basPortioner: 4`, `vintips: ""`, `bildUrl: null`). Egen `og:image`-extraktor körs server-side innan Claude — vi litar inte på att modellen plockar bild-URL korrekt eftersom HTML kan ha relativa paths.
- **Recept-detalj remountar via `key={recipe.id}`.** ESLint flaggade ursprungligen `setState` inuti `useEffect` för att synka portion-state mot fetchad data. Lösningen är "remount-on-data"-mönstret: `<RecipeView key={recipe.id} recipe={recipe} />` så `useState(recipe.basPortioner)` initieras färskt utan en effect-cycle.
- **Notion rich_text 2000-tecken-limit kan klippa långa ingredient-JSON.** Lade `chunkRichText()` som delar text i 2000-tecken-element i array:en. Notion concat:erar dem transparent vid läsning (`readText()` joins `plain_text` per element). Trösklen träffas sällan men finns nu.
...
**Levererar:**
- [x] `/api/mat/plan` — GET (`?weekStart=YYYY-MM-DD`), POST (skapa slot), PATCH, DELETE
- [x] `(warm)/v3/mat/planering/page.tsx` — vecka-grid (Mån–Sön × Lunch + Middag, **ingen frukost**), period-nav (←/→/Idag-titel-klick — samma mönster som fitness coach), ACC border + tint på dagens dag, slot-tiles: fyllda visar recept-namn + min, tomma visar "lägg till" med dashed border
- [x] Slot-väljar-modal (WarmModal): två-läges-toggle "Välj recept" / "Fritext". Recept-läget = sökbar lista från bibliotek (samma mönster som garden PlantPicker). Fritext-läget = namn + min-input. Sparar till `/api/mat/plan`
- [x] "Lägg på veckan"-knapp i recept-detalj → samma modal, recept förvalt, användaren väljer datum + slot (deep-link `?add=YYYY-MM-DD|Middag|<recipeId>`)
- [x] **Inköpslista-expand-panel** under vecka-grid: aggregera ingredienser från alla planerade recept inom veckan (skala via `portions / basePortions`), normalisera trivialt (`1 dl + 2 dl → 3 dl`, blandade enheter listas separat under samma namn), gruppera per butikskategori (Grönt/Mejeri/Kött & fisk/Skafferi/Övrigt — heuristisk via keyword-match i `src/lib/mat/shopping.ts`)
- [x] "Kopiera lista"-knapp (ACC) — klistrar formaterad plaintext till urklipp (`navigator.clipboard.writeText`), bock + "Kopierat" 800 ms feedback
- [x] Ingen persistens på inköpslista — regenereras varje render. Senare add (om användaren vill): `localStorage["mat:hidden:{ingredient}"]`-flagga per rad för "redan i skafferiet"
...
**Status / Observera:**
- **Befintlig "Veckomeny"-DB hade fel schema.** Användaren hade en gammal `Veckomeny`-DB med `Rätt`-title, `Måltid`-select (bara Middag-option), namnlös date-property och `Recept`-relation. Helt fel mot M0:s `Veckoplan`-schema (`Datum`/`Slot`/`EgetNamn`/`TidMin`/`Recept`). Valde **"Skapa fresh Veckoplan-DB"** över att adaptera koden — kören en gång (`scripts/create-mat-notion-dbs.mjs` med tom `NOTION_MAT_PLAN_DB`) gav en ny DB med exakt rätt schema. Gamla Veckomeny ligger kvar i Notion men är frikopplad. Lärdom: M0-init-skriptet är idempotent på env-var-närvaro, så det räcker att tomma raden i `.env.local` för att tvinga ny skapelse — utan att röra GitHub-secrets eller andra integrationer.
- **Kökschefen-sidan (NOTION_MAT_COACH_PAGE) saknade explicit `Connections → Träningscoach`.** Init-skriptet bailade med `object_not_found` när sidan skulle vara parent. Användaren delade manuellt — kunde inte automatiseras. Samma punkt som i M1:s observera-block (befintliga DB:er ärver inte connections från parent-page) gäller åt andra hållet: sidor under en redan-delad rot delar inte sina barn med integrationen.
- **Två commits, en delivery.** Splittade kodbasen mellan commit 1 (`feat(mat): M2 — veckoplanering` = API + week-grid + slot-modal + "Lägg på veckan") och commit 2 (`feat(mat): M2 — inköpslista-aggregat` = `src/lib/mat/shopping.ts` + ShoppingListPanel). Använde temp-strip av planering-page för commit 1, restore för commit 2 — så commit 1 inte importerade från en icke-existerande `shopping.ts`. Funkar men är klumpigt; för framtida sessioner med liknande layout-uppdelning är det enklare att introducera komponenten med en `null`-placeholder direkt och låta commit 2 byta ut den.
- **Slot-portioner finns inte i M2-schemat.** Slot-DB:n har `TidMin` (för fritext) men ingen portions-override. Konsekvens: inköpslista-aggregeringen skalar med `1.0`, dvs. använder recept-bas-portioner rakt av. Om man planerar Pasta carbonara (4 portioner) för 2 personer blir inköpslistan dimensionerad för 4. Lösning vid behov: lägg till `Portioner`-number-property i Veckoplan-DB:n + skicka in i `MealPlanInput` + multiplicera `scale = portioner / basPortioner` i `aggregateShoppingList`. Skipped för M2 — användarens nuvarande veckor är typiskt familje-storlek = recept-bas.
- **Butikskategori-heuristiken är inte perfekt.** "krossade tomater" hamnar i Grönt (matchar `tomat`) trots att skafferi vore mer korrekt — `Grönt`-listan testas före `Skafferi`. Likadant rankas första matchning vinner per kategori-ordning. Räcker för 80%-fallet; om listan blir störande kan man flytta `krossade tomater`/`passerade tomater`-keywords högst upp i `Skafferi` och kolla dem före `Grönt`. Bestämde att inte överoptimera — användaren ser kategorin på listan och hittar saken oavsett.
- **Enhets-normalisering: cl och msk delas inte ihop.** Aggregeringen samlar volym i ml-bas (l/dl/cl/ml → ml), så `5 cl + 5 cl = 1 dl` blir korrekt. Men icke-volym-enheter (msk/tsk/st/klyfta) hålls separata i `otherUnits` — `2 msk + 1 dl olja` blir `1 dl + 2 msk olja` (två rader på samma namn), inte konverterat till en. Det är rätt: 1 msk ≈ 15 ml men i recept blir det 1 msk för precision. Användarens hjärnliga uppskattning räcker när hen handlar.
- **Veckoplan-relation pekar mot Recept-DB:s `data_source_id`, inte `database_id`.** Notion API 2025-09-03 kräver det. Init-skriptet löser det via `dataSourceId(RECIPES_DB_ID)`-helpern. Att skapa raden via API:n: `properties.Recept = { relation: [{ id: recipeId }] }` — page-ID:t räcker, ingen ds-info per rad.
