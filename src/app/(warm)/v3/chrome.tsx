...
import { WarmUserProvider, useWarmUser } from "@/lib/warm/user";
...
export default function WarmV3Chrome({
  user,
  children,
}: {
  user: UserProfile;
  ...
  return (
    <WarmThemeProvider>
      <WarmTopBarProvider>
        <WarmUserProvider user={user}>
          <WarmV3ChromeInner>{children}</WarmV3ChromeInner>
        </WarmUserProvider>
      </WarmTopBarProvider>
    </WarmThemeProvider>
  );
...
function WarmV3ChromeInner({ children }: { children: ReactNode }) {
  ...
  // Om aktuell URL pekar på en flik som användaren inte har — fall tillbaka
  // till första tillåtna fliken så TabBar/Sidebar markerar rätt aktiv-state.
  const rawTab = activeTab(pathname);
  const tab = allowedTabs.includes(rawTab) ? rawTab : (allowedTabs[0] ?? "hem");
