...
// Fullskärm-chat mot /api/fitness/chat (SSE). Tool-use renderas som inline-chip.
// Coachen kan anropa plan-CRUD-tools direkt via samtalet och generera plan-
// förslag via Claude. Manuell kalender-vy + AI-knappar finns kvar på
// `/v3/fitness/coach/plan` (ingång via "Planera"-knapp i headern).
...
import { CalendarIcon } from "@/components/warm/icons/fit";
...
const QUICK_PROMPTS = [
  "Vad ska jag köra ikväll?",
  "Planera kommande vecka",
  "Hur ser min återhämtning ut?",
  "Föreslå ett pass på torsdag",
];
...
function ToolChip({ tool }: { tool: ToolPart }) {
  ...
  const label =
    tool.name === "create_planned_workout" ? "Skapar pass"
    : tool.name === "update_planned_workout" ? "Uppdaterar pass"
    : tool.name === "archive_planned_workout" ? "Arkiverar pass"
    : tool.name === "list_planned_workouts" ? "Listar planerade pass"
    : tool.name === "get_recent_workouts" ? "Läser senaste pass"
    : tool.name === "generate_week_plan" ? "Genererar veckoplan"
    : tool.name === "generate_single_workout" ? "Förslår pass"
    : tool.name;
  ...
  if (tool.ok && tool.result) {
    if (Array.isArray(tool.result)) blurb = ` → ${tool.result.length} pass`;
    else if (typeof tool.result === "object" && tool.result !== null) {
      const r = tool.result as Record<string, unknown>;
      if ("passnamn" in r && typeof r.passnamn === "string") blurb = ` → "${r.passnamn}"`;
      else if ("items" in r && Array.isArray(r.items)) blurb = ` → ${r.items.length} förslag`;
      else if ("item" in r && typeof r.item === "object" && r.item !== null) {
        const it = r.item as Record<string, unknown>;
        if ("passnamn" in it && typeof it.passnamn === "string") blurb = ` → "${it.passnamn}"`;
      }
    }
  }
  ...
  return (
    <Tile t={t} hi style={{ padding: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <span style={lab(t)}>Kontext laddat till coachen</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 8 }}>
        {[
          { label: "CTL", value: fmt(ctx?.load.ctl, 0) },
          { label: "ATL", value: fmt(ctx?.load.atl, 0) },
          { label: "TSB", value: fmt(ctx?.load.tsb, 0) },
          { label: "Pass i fönstret", value: ctx?.recentWorkouts ?? "–" },
        ].map(({ label, value }) => (
      ...
      {ctx?.sourceFile && (
        <div style={{ ...ital(t, 11, t.dim), marginTop: 8 }}>
          källa: {ctx.sourceFile}
        </div>
      )}
    </Tile>
  );
...
function FitnessCoachInner() {
  ...
  const ctxSwr = useSWR<ContextSummary>("/api/fitness/chat", fetcher, { revalidateOnFocus: false, refreshInterval: 0 });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100dvh - 190px)", minHeight: 460, paddingBottom: 8 }}>
      <DetailHero
        ...
        eyebrow="COACH"
        title="Coachen,"
        italicTail="planera & fråga."
        subtitle={
          ctx
            ? `CTL ${ctx.load.ctl?.toFixed(0) ?? "–"} · ATL ${ctx.load.atl?.toFixed(0) ?? "–"} · TSB ${ctx.load.tsb != null && ctx.load.tsb >= 0 ? "+" : ""}${ctx.load.tsb?.toFixed(0) ?? "–"}`
            : "laddar kontext…"
        }
        right={
          <div style={{ display: "inline-flex", gap: 6 }}>
            <Link
              ...
              style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: body, fontSize: 11, fontWeight: 600, background: t.paper, color: t.ink, border: `1px solid ${t.line}`, borderRadius: 999, padding: "5px 11px", cursor: "pointer", textDecoration: "none" }}
            >
              <CalendarIcon size={11} color={t.mute} />
              Planera
            </Link>
            <button
              type="button"
        ...
        {notReady && (
          <Tile t={t}>
            <p style={{ fontFamily: body, fontSize: 13, color: t.mute, lineHeight: 1.55 }}>
              Coach-chatten kräver att <code>ANTHROPIC_API_KEY</code> är satt.
            </p>
          </Tile>
        )}
        ...
        {isEmpty && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: t.paper, border: `1px solid ${t.line}`, borderRadius: 14 }}>
              ...
              <div>
                <div style={{ ...num(t, 14, 500), color: t.ink, lineHeight: 1.2 }}>Fråga coachen</div>
                <div style={{ ...ital(t, 11, t.dim), marginTop: 2 }}>Senaste pass · PMC (CTL/ATL/TSB) · planerade pass · plan-CRUD via samtal</div>
              </div>
            </div>
